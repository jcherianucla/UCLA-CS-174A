...
//Draws the text on screen for the story
Debug_Screen.prototype.draw_story_line = function(model_transform, text)
{
	model_transform = mult(model_transform, translation(0, 15, -12));
...
Debug_Screen.prototype.display = function(time)
{
	...
	//Hold start point for ease of access
	var start_point = model_transform;
	...
	for( var i = 0, idx = this.start_index; i < 4 && i < strings.length; i++, idx = (idx + 1) % strings.length )
	{
		...
		//Draw the respective lines at their respected times
		if(anim_frame_num <= 100 && anim_frame_num > 0)
		{
			this.draw_story_line(start_point, "I sense a trespasser!");
		} else if(anim_frame_num > 110 && anim_frame_num <= 230)
		{
			this.draw_story_line(start_point, "YOU SHALL NOT PASS!");
		} else if(anim_frame_num > 230 && anim_frame_num <= 350)
		{
			this.draw_story_line(start_point,"Let me pass beast!");
		} else if(anim_frame_num > 350 && anim_frame_num <= 470)
		{
			this.draw_story_line(start_point,"Or face the wrath");
		} else if(anim_frame_num > 1200 && anim_frame_num <= 1300)
		{
			this.draw_story_line(start_point, "I warned you");
		}
	}
